...
while($wynik= mysqli_fetch_assoc($pytanie))
	...
	$cena_netto_za_sztuke[$ilosc_pozycji] = $cena_netto_za_sztuke[$ilosc_pozycji]/$kurs_euro;
...
if($submit == 'Wystaw')
	...
	if($nowa_informacja_o_fakturze != '')	
		{
		echo '<div class="text_green" align="center">Dodano now± informację o fakturze.</div>';
	...
	if($pozycja[$x] == 'on')
		...
		mysqli_query($conn, "INSERT INTO fv_pozycje (`fv_id`, `nr_fv`, `zamowienie_id`, `nabywca_id`, `pozycja`, `nazwa_produktu`, `jednostka`, `ilosc`, `cena_netto`, `vat`, `cena_brutto`, `wartosc_netto`, `wartosc_brutto`) 
										values ('$fv_id', '$numer_faktury', '$zamowienie_id', '$klient_id', '$x', '$nazwa_produktu[$x]', '$jednostka[$x]', '$ilosc_sztuk[$x]', '$cena_netto_za_sztuke[$x]', '$stawka_vat_np[$x]', '$cena_brutto_za_sztuke[$x]', '$wartosc_netto[$x]', '$wartosc_brutto[$x]');");
		...
		echo '<div align="center" class="text_duzy_niebieski">Faktura proforma EURO '.$numer_faktury.' dla zamówienia '.$nr_zamowienia.' została wystawiona.</div><br>';
	...
	echo '<FORM action="index.php?page=fv_wystaw_proforme_euro&zamowienie_id='.$zamowienie_id.'" method="post">';
	...
	echo '<div align="center" class="text_ogromny">Wystaw fakturę proformę EURO do zamówienia : <font color="blue">'.$nr_zamowienia.'</font></div><br>';
	...
	echo '<tr align="center" class="text"><td width="50%">';
		...
		if($nabywca_forma_platnosci == '') echo '<tr class="text" align="left"><td>Forma płatności:</td><td colspan="2"><font color="red">BRAK SPOSOBU PŁATNO¦CI</font></td></tr>';
			...
			echo '<tr class="text" align="left"><td>Informacja o fakturze:</td><td colspan="2">';
			...
			echo '<select name="informacja_o_fakturze" class="pole_input" style="width: 320px">';
			echo '<option></option>';
	...
	echo '<td width="50%" valign="bottom">';
		...
		echo '<tr class="text" align="right"><td colspan="2" align="left">LUB&nbsp;&nbsp;<input autocomplete="off" type="text" size="40" maxlength="150" title="Informacja o fakturze" alt="Informacja o fakturze" class="pole_input" name="nowa_informacja_o_fakturze" value="'.$nowa_informacja_o_fakturze.'"></td></tr>';
		echo '</table>';
	echo '</td></tr>';
	...
	echo '<td width="100%" colspan="2">';
		...
		for($x = 1; $x<=$ilosc_pozycji; $x++)
			...
			//$cena_netto_za_sztuke[$x] = $cena_netto_za_sztuke[$x]/$kurs_euro;
			$cena_netto_za_sztuke[$x] = number_format($cena_netto_za_sztuke[$x], 2,'.','');
			echo '<td align="right">'.$cena_netto_za_sztuke[$x].'</td>';
			
			if($stawka_vat_np[$x] == 'NP') echo '<td>'.$stawka_vat_np[$x].'</td>';
			else echo '<td>'.$stawka_vat[$x].'%</td>';
			...
			echo '<td align="right">'.$wartosc_brutto[$x].'</td>';
		...
		$slownie_kwota = kwotaslownieeuro($SUMA_BRUTTO);
		echo $slownie_kwota;
		echo '</td>';
		echo '<td align="right">Razem w EUR: </td>';
		$SUMA_NETTO = number_format($SUMA_NETTO, 2,'.',' ');
		$SUMA_BRUTTO = number_format($SUMA_BRUTTO, 2,'.',' ');
