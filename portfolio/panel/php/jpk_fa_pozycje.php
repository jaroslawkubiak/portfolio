<?php
echo '<div class="text_duzy_niebieski" align="center">Wygenerowano plik JPK_FA pozycje</div><br>';
...
if(($miesiac_od != '') && ($miesiac_do != '') && ($rok_od != '') && ($rok_do != ''))
	...
	while($wynik33= mysqli_fetch_assoc($pytanie33))
		...
		// KONIEC /tylko do obliczenia sumy brutto przed tabela
		}

	
		$SUMA_BRUTTO = number_format($SUMA_BRUTTO, 2,'.',' ');
		echo '<br><div align="left" class="text">Wygenerowano plik JPK_FA za okres od : <font color="red">'.$TABELA_MIESIECY[$miesiac_od].' '.$rok_od.'</font> do :  <font color="red">'.$TABELA_MIESIECY[$miesiac_do].' '.$rok_do.'</font>.';
		echo $tabulator.' Liczba wierszy <font color="red">'.$liczba_wierszy.'.'.$tabulator.'</font> Wartość wierszy <font color="red">'.$SUMA_BRUTTO.' '.$waluta.'</font></div><br>';
