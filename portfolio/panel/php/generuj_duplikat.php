...
while($wynik33= mysqli_fetch_assoc($pytanie33))
	...
	$termin_platnosci_do_duplikatu=$wynik33['termin_platnosci'];
	$wartosc_netto_fv_do_duplikatu=$wynik33['wartosc_netto_fv'];
	$wartosc_brutto_fv_do_duplikatu=$wynik33['wartosc_brutto_fv'];
	$vat_faktura_do_duplikatu=$wynik33['vat'];
...
while($wynik22= mysqli_fetch_assoc($pytanie22))
	...
	$sprzedawca_opis=$wynik22['opis']; // info o fv na stale z ustawien
...
// jezeli przeztwarzamy duplikat to bierzemy pozycje z faktury - duplikat nie ma pozycji

$pytanie3334 = mysqli_query($conn, "SELECT * FROM fv_naglowek WHERE duplikat_do_faktury = '".$fv_id."';");
while($wynik3334= mysqli_fetch_assoc($pytanie3334))
	{
	$id_faktury_do_duplikatu=$wynik3334['id'];
	}	
...
if($id_faktury_do_duplikatu != '') $pytanie333 = mysqli_query($conn, "SELECT * FROM fv_pozycje WHERE fv_id = ".$id_faktury_do_duplikatu." ORDER BY pozycja ASC;");
else $pytanie333 = mysqli_query($conn, "SELECT * FROM fv_pozycje WHERE fv_id = ".$fv_id." ORDER BY pozycja ASC;");
...
$pdf->SetXY(5,40);
$duplikat_data = date('d.m.Y', $time);
$napis_duplikat = 'FAKTURA DUPLIKAT wystawiona dnia '.$duplikat_data.'.'; //
$napis_duplikat = iconv('UTF-8','windows-1250//TRANSLIT', $napis_duplikat);
$pdf->Multicell(160, 10, $napis_duplikat, 0, 'L' ,0); //piewszy parametr - szerokosc, drugi wysokosc, trzeci teskt, czwarty czy ma byc ramka, 0 - brak, 1  -ramka. C - wyśrodkowane, ostatni parametr to tło
$pdf->SetXY(165,40);
$napis_nr_fv = 'Nr: '.$nr_fv;
$pdf->Multicell(40, 10, $napis_nr_fv, 0, 'R' ,0); 
$pdf->SetFont('arial_ce','', 8);
...
for($x = 1; $x <= $ilosc_pozycji; $x++)
	...
	if(($kurs_euro != '') && ($kurs_euro != 0))
		{
		$cena_netto[$x] = $cena_netto[$x]/$kurs_euro;
		$cena_brutto[$x] = $cena_brutto[$x]/$kurs_euro;
		$wartosc_netto[$x] = $wartosc_netto[$x]/$kurs_euro;
		$wartosc_brutto[$x] = $wartosc_brutto[$x]/$kurs_euro;
		}
	...
	$pdf->Multicell($szer_ramki, $wys_ramki, $VAT[$x], $obramowanie, 'C' ,0);
...
if(($kurs_euro != '') && ($kurs_euro != 0))
	...
	$do_zaplaty2 = $do_zaplaty2/$kurs_euro;
	}
else 
	{
	$waluta_na_fv = 'Razem w PLN';	
	$kwota_brutto_slownie = kwotaslownie($wartosc_brutto_fv);
	}
...
$pdf->Multicell(30, $wys_ramki, $waluta_na_fv, 0, 'R' ,0);
...
if(($kurs_euro != '') && ($kurs_euro != 0)) $wplacono = $wplacono/$kurs_euro;
$wplacono = number_format($wplacono, 2,'.',' ');
$pdf->Multicell($szer_ramki, $wys_ramki, $wplacono, $obramowanie, 'C' ,0);

$szer_poczatkowa += $szer_ramki;
$pdf->SetXY($szer_poczatkowa, $wys_poczatkowa2);
if(($kurs_euro != '') && ($kurs_euro != 0)) $waluta_na_fv2 = 'EUR';	else $waluta_na_fv2 = 'PLN';	
$do_zaplaty2 = number_format($do_zaplaty2, 2,'.',' ');
$pdf->Multicell($szer_ramki, $wys_ramki, $do_zaplaty2, $obramowanie, 'C' ,0);
$szer_poczatkowa += $szer_ramki;
$pdf->SetXY($szer_poczatkowa, $wys_poczatkowa2);
$pdf->Multicell(30, $wys_ramki, $waluta_na_fv2, $obramowanie, 'C' ,0);
...
/*
	$do_zaplaty2=$wynik33['do_zaplaty'];
*/
...
$nazwa_pliku = 'faktura_duplikat_'.$nr_fv.'.pdf';
$pdf->Output("../panel_dane/faktury_duplikat/".$nazwa_pliku."", "F");
