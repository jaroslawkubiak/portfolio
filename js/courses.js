...
coursesList.forEach((item, index) => {
  ...
  let html = `
    <li class="courses-item-wrapper">
        <div class="courses-item-circular-progress" title="${complete ? 'Done' : 'In progress'}">
            <span class="courses-item-progress-value" data-start="${item.percent}"></span>
        </div>
        <div class="courses-item-title">
            <a href="${item.udemy}" target="_blank" title="View course details">${item.name}</a>
        </div>
