...
function update() {
  ...
  if (positionFromTop < -1500 && !goUp)
    pageBody.insertAdjacentHTML('afterbegin', `<div class="arrow-go-up" id="goUp" title="Go to top">${arrowUp}</div>`);
}
...
const animateCoursesCircle = function () {
  ...
  circularProgress.forEach((circle, index) => {
    ...
    const progress = setInterval(() => {
      if (progressCounter === progressEndValue || !progressEndValue) clearInterval(progress);
...
const animateSoftSkills = function () {
  ...
  skillContainerHeight = Number(skillContainerHeight.substring(0, skillContainerHeight.length - 2));
